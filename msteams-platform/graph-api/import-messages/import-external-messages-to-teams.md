...
To start migrating a user's message history from any third-party platform to Teams, use an existing channel or chat. An existing channel or chat can be in any state (active, inactive, or archived) and can be created before or after the migration process starts.
